...
class SidepanelManager {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.clearTranscript());
    document
      .getElementById("testApisBtn")
      .addEventListener("click", () => this.testApis());
    document
    ...
    document
      ...
      .addEventListener("click", () => this.toggleConfig());
    document
    ...
    document
      ...
      .addEventListener("click", () => this.hideExportModal());
    document.querySelectorAll(".btn-export").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        this.exportTranscript(e.target.closest(".btn-export").dataset.format)
      );
    });
    document
    ...
    document
      ...
      .addEventListener("click", () => this.hideToast("error"));
    document
      .getElementById("closeSuccessToast")
      .addEventListener("click", () => this.hideToast("success"));
    ...
    try {
      ...
      if (isCurrentlyRecording) {
        ...
        if (!configResponse.config || !configResponse.config.apiKey) {
          this.showToast(
            "Please configure an API key in settings (⚙️) before recording.",
            "error"
          );
          return;
        }

        const timerBody = document.getElementById("timerContainer");
        ...
        timerBody.appendChild(equalizer);
        ...
        if (success) {
          ...
          this.showToast(
            "Recording started! Note: Tab audio may be muted during recording (this is normal).",
            "success"
          );
        } else {
  ...
  updateRecordingUI(isRecording) {
    ...
    if (isRecording) {
      ...
      recordBtn.innerHTML =
        '<i class="fas fa-play"></i><span>Start Recording</span>';
    ...
    try {
      ...
      if (response.config) {
        document.getElementById("apiProvider").value =
          response.config.provider || "google";
  ...
  async updateStatus() {
    try {
      ...
      if (!status.tabCaptureAvailable) {
        this.showToast(
          "Tab capture API not available. Please reload the extension.",
          "error"
        );
      }
      ...
      if (!status.storageAvailable) {
        this.showToast(
          "Storage API not available. Extension may not work properly.",
          "error"
        );
      }
  ...
  async loadTranscript() {
    try {
      ...
      console.log(
        `🔍 Checking transcript: current=${this.currentTranscript.length}, new=${newTranscript.length}`
      );

      // Always update if we have new entries
      if (newTranscript.length > this.currentTranscript.length) {
        console.log(
          `📥 Loaded transcript: ${this.currentTranscript.length} → ${newTranscript.length} entries`
        );
      ...
      // Also update if same length but different content (in case entries were modified)
        ...
        newTranscript.length === this.currentTranscript.length &&
  ...
  handleNewTranscriptEntry(entry) {
    console.log(
      "📥 Received new transcript entry in sidepanel:",
      entry.text.substring(0, 50) + "..."
    );

    // Check if this entry already exists to avoid duplicates
    const existingEntry = this.currentTranscript.find(
  ...
  async exportTranscript(format) {
    try {
      ...
      if (response.success && response.data) {
        ...
        this.hideExportModal();
        this.showToast(
          `Transcript exported as ${format.toUpperCase()}`,
          "success"
        );
      }
    } catch (error) {
  ...
  async testApis() {
    try {
      ...
      message += `API Key: ${
        configResponse.config?.apiKey ? "Configured ✅" : "Not configured ❌"
      }\n`;
  ...
  async requestPermissions() {
    try {
      ...
      if (result.success && result.granted) {
        ...
        this.showToast(
          "Failed to get permissions: " + (result.error || "Unknown error"),
          "error"
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (this.isRecording) {
        ...
        console.log(
          ...
          this.localAudioChunks?.length || 0
        );
      } else {
      ...
      this.showToast(
        "Diagnostics completed - check console for details",
        "success"
      );
    } catch (error) {
    ...
    try {
        ...
        chrome.tabCapture.capture(
          ...
          (capturedStream) => {
            ...
            if (err) {
              ...
              if (
                err.message.includes("activeTab permission") ||
                err.message.includes("not been invoked")
              ) {
                ...
                err.message.includes("Chrome pages cannot be captured")
              ) {
                const chromePageMessage =
                  `❌ Cannot record Chrome system pages!\n\n` +
                  `🔄 Please:\n` +
              ...
              reject(
                new Error(
                  "Failed to capture tab audio. Make sure the tab is playing audio and try refreshing the page."
                )
              );
            } else {
      ...
      this.localMediaRecorder.ondataavailable = (event) => {
        ...
        if (event.data.size > 0) {
          ...
          console.log(
            "📤 Processing 30-second audio chunk for transcription..."
          );
  ...
  async processChunkWithOverlap(currentChunk) {
    try {
      ...
      if (this.overlapBuffer) {
        console.log(
          "🔄 Combining with 3-second overlap from previous chunk..."
        );
  ...
  async createOverlapBuffer(audioChunk) {
    try {
      ...
      const overlapSize = Math.floor(
        arrayBuffer.byteLength * OVERLAP_DURATION_RATIO
      );

      if (overlapSize > 1000) {
        // Only create buffer if substantial data
        ...
        console.log("💾 Created 3-second overlap buffer:", {
          ...
          ratio:
            ((overlapData.byteLength / arrayBuffer.byteLength) * 100).toFixed(
              1
            ) + "%",
        });
      } else {
  ...
  async sendAudioChunk(audioBlob) {
    try {
      ...
      if (audioBlob.size < 1000) {
        console.warn(
          "⚠️ Audio chunk too small, might be silence:",
          audioBlob.size,
          "bytes"
        );
      ...
      console.log(
        "📤 Sending audio chunk to background for transcription... Size:",
        base64Audio.length,
        "chars"
      );
      ...
      if (
        ...
        console.log(
          "🔄 Scheduled multiple refreshes to catch transcript update"
        );
      } else if (response && response.success && !response.transcription) {
        console.log(
          "⚪ No transcription text returned (might be silence or API returned empty)"
        );
        ...
        if (
          !response.error.includes("Failed to get file URI") &&
