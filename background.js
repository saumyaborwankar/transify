...
class AudioTranscriptionManager {
  ...
  async init() {
    ...
    if (!chrome.tabCapture) {
      console.error(
        "Tab capture API is not available. Extension may not work properly."
      );
    ...
    if (!chrome.storage) {
      console.error(
        "Storage API is not available. Extension may not work properly."
      );
    }
  ...
  async startRecording() {
    ...
    try {
      ...
      if (!tab) {
        throw new Error(
          "No active tab found. Please ensure you have an active tab."
        );
      }
  ...
  async transcribeAudio(audioBase64) {
    ...
    try {
      ...
      if (result && result.trim()) {
        ...
        console.log(
          `✅ Transcription successful with ${this.apiConfig.provider}:`,
          result
        );
        ...
        console.log(
          "⚪ API returned empty result (likely silence or no speech detected)"
        );
      ...
      if (
        error.message.includes("Failed to get file URI") ||
      ...
      if (this.retryCount >= this.maxRetries) {
        ...
        console.log(
          "⚠️ Max retries reached, returning null instead of throwing error"
        );
        return null;
      }
  ...
  async callGeminiAPI(audioBase64, config) {
    ...
    console.log(
      "🔍 Calling Gemini 2.5 Flash API with audio size:",
      audioBase64.length
    );
    ...
    try {
      ...
      if (transcript && transcript.trim()) {
        console.log(
          "✅ Direct Gemini transcription successful, skipping file upload fallback"
        );
        return transcript;
      }
      ...
      console.log(
        "🔄 Direct method returned empty result, trying file upload method..."
      );
      ...
      if (
        ...
        throw new Error(
          "Gemini API access denied. Please check: 1) API key is valid, 2) Gemini API is enabled in Google Cloud Console, 3) File Service permissions are granted"
        );
      }
  ...
  async callGeminiDirectAPI(audioBase64, config) {
    try {
      ...
      if (transcript) {
        ...
        console.log(
          "⚪ Direct method returned empty transcript (likely silence)"
        );
        return null; // Return null to trigger fallback
      }
    } catch (error) {
  ...
  async callGeminiFileUploadAPI(audioBase64, config) {
    try {
      ...
      const uploadResponse = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/files?key=${config.apiKey}`,
        {
          method: "POST",
          body: formData,
        }
      ...
      console.log(
        "📡 Gemini transcription response status:",
      ...
      if (!transcribeResponse.ok) {
        ...
        throw new Error(
          `Gemini transcription failed: ${transcribeResponse.status}`
        );
      }
      ...
      try {
        await fetch(
          `https://generativelanguage.googleapis.com/v1beta/${fileUri}?key=${config.apiKey}`,
          {
            method: "DELETE",
          }
        );
  ...
  async callGoogleSpeechAPI(audioBase64, config) {
    ...
    console.log(
      "🔍 Calling Google Speech API with audio size:",
      audioBase64.length
    );
  ...
  async callOpenAIWhisperAPI(audioBase64, config) {
    ...
    console.log(
      "🔍 Calling OpenAI Whisper API with audio size:",
      audioBase64.length
    );
  ...
  notifySidepanelOfNewTranscript(entry) {
    try {
      console.log(
        "📤 Notifying sidepanel of new transcript entry:",
